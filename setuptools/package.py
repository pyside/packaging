...
if sys.platform == "win32":
    ...
    def make_package(pkg_version, script_dir, sources_dir, build_dir, install_dir,
        py_version, pack_examples, qtinfo, logger):
...
if __name__ == "__main__":
    ...
    try:
        ...
        optparser.add_option("-s", "--script-dir", dest="script_dir",
                             default=None, help="Specify script directory")
        optparser.add_option("-b", "--build-dir", dest="build_dir",
                             default=None, help="Specify build directory")
        ...
        if not os.path.exists(options.build_dir):
            logger.error("Please specify the build directory with --build-dir parameter")
            sys.exit(1)
